...
真實車輛位置
  └─ 車機回傳延遲          （不可控）
      └─ TDX 資料更新週期   ≤ 60s   ← 瓶頸在這裡，不可控
          └─ 後端拉取週期    30s    ← 只有這段可控，且被 §0.2 的配額鎖死
              └─ App 輪詢週期 15s
```

**結論**：後端 30 秒拉取是合理的——TDX 本來就每分鐘才更新，拉更快不會更新鮮，而
§0.2 實測的配額（每 30 秒 5 次）也不允許更快。但 **App 上顯示的位置對「真實世界」的誤差最壞情況仍會超過 30 秒。這是資料源的物理限制，不是實作問題。**
...
**配額為每 30 秒 5 次的固定窗口。** 第一次呼叫於 t=0 開窗，5 次在 3.3 秒內用完，
t=30 窗口重置——因此 t=19.7 仍被擋、t=35.4 通過，完全吻合。

**對輪詢間隔的意義**（每輪拉 A1 + N1 共 2 次）：

| 間隔 | 每 30 秒窗口的呼叫數 | 餘裕 | 判斷 |
|---|---|---|---|
| 15 秒 | 4 次 | 1 次 | ⚠️ 純輪詢可行，但**零餘裕**，靜態同步一插入就塞車 |
| **30 秒** | **2 次** | **3 次** | ✅ **採用** |
| 60 秒 | 1 次 | 4 次 | 過於保守，且已超過 TDX 自身的更新週期 |

> 早期版本曾誤判「15 秒必然超標」。實際上 15 秒間隔在任一 30 秒窗口內
> 最多只有 2 輪輪詢 = 4 次呼叫，並未超過 5 次。選 30 秒的真正理由是**餘裕**：
> 每日靜態同步要 4 次呼叫，加上 token 更新與重試，15 秒的 1 次餘裕不夠用。

**因應措施**：

1. `POLL_INTERVAL_MS` 預設改為 **30000**
2. **`src/tdx/pacer.ts` 在唯一出口強制節流**——配額低到「請每個呼叫端自己記得」
   是不可靠的做法，只要有人多寫一個 `Promise.all` 就會吃掉整個窗口
   （原本的 `staticStore` 正是如此，且啟動時會與 `liveCache` 疊加成 6 個並發請求）
3. `liveCache` 保留 429 指數退避作為最後防線
4. `/healthz` 曝露節流統計，線上出問題時可立即判斷是否為配額所致
5. 若日後需要更高頻率，須申請 TDX 進階服務
